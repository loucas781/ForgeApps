...
const OUTPUT_PATH = 'docs/assets/appstore-release-history.json';
...
const toReleaseSnapshot = (release, app) => {
  const version = release?.version || release?.versionDisplay || release?.versionString || null;
...
const mergeReleases = (existingReleases = [], fetchedReleases = []) => {
  const releases = [];
  ...
  existingReleases.forEach(addRelease);
...
const fetchVersionHistory = async (app) => {
  ...
  return findVersionHistory(platformAttributes)
    .map((release) => toReleaseSnapshot(release, app))
    .filter(Boolean);
};
...
const run = async () => {
  ...
  for (const app of APPS) {
    const existingReleases = Array.isArray(prior?.apps?.[app.key]) ? prior.apps[app.key] : [];

    if (!app.appId) {
      nextApps[app.key] = existingReleases;
      continue;
    }

    try {
      const fetchedReleases = await fetchVersionHistory(app);
      nextApps[app.key] = mergeReleases(existingReleases, fetchedReleases);
    } catch (error) {
  ...
  await writeFile(OUTPUT_PATH, `${JSON.stringify({
    source: 'app-store-web-version-history',
    country: COUNTRY,
